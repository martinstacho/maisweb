...
export default async function PreInstituciePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg)', color: 'var(--fg)' }}>
      ...
      {/* Hero */}
      <section className="relative overflow-hidden py-28 px-6 noise" style={{ background: 'var(--bg)' }}>
        <div className="absolute inset-0 hero-grid-sm pointer-events-none" />
        ...
        <div className="relative z-10 mx-auto max-w-5xl text-center">
          <FadeIn>
            <div className="kicker justify-center mb-6">Pre inštitúcie</div>
            <h1 className="font-display text-[52px] md:text-[80px] leading-[0.92] mb-6">
              <span className="gradient-text">Akademický systém,</span>
              <br />
              <span style={{ color: 'var(--fg)' }}>ktorý naozaj funguje</span>
            </h1>
            <p className="text-[17px] md:text-[19px] leading-relaxed max-w-2xl mx-auto mb-10" style={{ color: 'var(--fg-2)' }}>
              MAIS je overený systém pre správu celej akademickej agendy. Nasaďte ho a zabudnite na papierovú byrokraciu.
            </p>
            <Link href={`/${locale}/kontakt`}
              className="btn-primary rounded-xl px-8 py-4 text-[15px] font-medium inline-flex items-center gap-2.5">
              Požiadať o demo
      ...
      {/* Why MAIS */}
      <section className="border-t px-6 py-24" style={{ borderColor: 'var(--line)', background: 'var(--bg-2)' }}>
        <div className="mx-auto max-w-7xl">
          <FadeIn className="mb-12 text-center">
            <div className="kicker justify-center mb-4">Prečo MAIS</div>
            <h2 className="font-display text-[36px] md:text-[52px]" style={{ color: 'var(--fg)' }}>
              Čo nás odlišuje
            </h2>
          </FadeIn>
            ...
            {whyItems.map((item, i) => (
              <FadeIn key={item.title} delay={i * 0.07}>
                <div className="bento-card p-7">
                  ...
                  <div className="tech-dot mb-4" />
                  <div className="font-semibold text-[15px] mb-2" style={{ color: 'var(--fg)' }}>{item.title}</div>
                  <div className="text-[13px] leading-relaxed" style={{ color: 'var(--fg-3)' }}>{item.desc}</div>
                </div>
              </FadeIn>
            ))}
          </div>
      ...
      <section className="px-6 py-24">
        <div className="mx-auto max-w-4xl">
          <FadeIn className="mb-14 text-center">
            <div className="kicker justify-center mb-4">História</div>
            <h2 className="font-display text-[36px] md:text-[52px]" style={{ color: 'var(--fg)' }}>
              22 rokov <span className="gradient-text">vývoja</span>
            </h2>
          </FadeIn>
          <div className="relative">
            <div className="absolute left-7 top-0 bottom-0 w-px" style={{ background: 'var(--line)' }} />
              ...
              {timeline.map((item, i) => (
                <FadeIn key={item.year} delay={i * 0.08}>
                  <div className="flex items-start gap-6">
                    ...
                    <div className="pb-4">
                      <div className="mono text-[11px] tracking-wider mb-1" style={{ color: 'var(--orange)' }}>{item.year}</div>
                      <div className="font-semibold text-[15px] mb-1" style={{ color: 'var(--fg)' }}>{item.label}</div>
                      <div className="text-[13px] leading-relaxed" style={{ color: 'var(--fg-3)' }}>{item.desc}</div>
                    </div>
                  </div>
                </FadeIn>
        ...
        <FadeIn>
          <div className="mx-auto max-w-3xl rounded-3xl p-px" style={{ background: 'linear-gradient(135deg, oklch(0.55 0.22 40 / 0.5), oklch(0.4 0.15 40 / 0.2) 50%, oklch(0.55 0.22 40 / 0.5))' }}>
            <div className="relative overflow-hidden rounded-3xl p-12 text-center" style={{ background: 'linear-gradient(180deg, oklch(0.22 0.018 40 / 0.95), oklch(0.16 0.014 40 / 0.95))' }}>
              ...
              <div className="kicker justify-center mb-6">Ste pripravení?</div>
              <h2 className="font-display text-[32px] md:text-[44px] leading-tight mb-4" style={{ color: 'var(--fg)' }}>
                Dohodnime si <span className="gradient-text">prezentáciu</span>
              </h2>
